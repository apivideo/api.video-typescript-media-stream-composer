...
export class MediaStreamComposer {
    ...
    private calculateCoverDimensions(containerDimentions: Resolution, elementDimentions: Resolution): Resolution & Position & { radius?: number } {
        let width, height;
    ...
    private calculateContainDimensions(containerDimentions: Resolution, elementDimentions: Resolution): Resolution & Position & { radius?: number } {
        let width, height;
        if (elementDimentions.width / containerDimentions.width > elementDimentions.height / containerDimentions.height) {
            width = containerDimentions.width;
            height = elementDimentions.height * containerDimentions.width / elementDimentions.width;
            ...
            height = containerDimentions.height;
            width = height * elementDimentions.width / elementDimentions.height;
        };
        let x = 0, y = 0;
        if (width < containerDimentions.width) {
            x = (containerDimentions.width - width) / 2;
    ...
    private calculateFixedDimensions(containerDimentions: Resolution, elementDimentions: Resolution, targetDimensions: Resolution, position?: Position, mask?: Mask): Resolution & Position & { radius?: number } {
        ...
        return { radius, width, height, x: position?.x || 0, y: position?.y || 0 }
    }
    ...
    private buildStreamDisplaySettings(id: string, mediaStream: MediaStream, options: StreamOptions): StreamDisplaySettings {
        ...
        if (options.position === "contain") {
            pos = this.calculateContainDimensions(containerResolution, streamResolution);
        } else if (options.position === "cover") {
            pos = this.calculateCoverDimensions(containerResolution, streamResolution);
        } else {
    ...
    private mouseMoveWhileDragging(e: MouseEvent) {
        ...
        if (this.mouseAction === "move") {
            this.updateStream(this.drag.streamId, {
                ...
                width: stream.displaySettings.width,
                height: stream.displaySettings.height,
                position: "fixed"
            });
